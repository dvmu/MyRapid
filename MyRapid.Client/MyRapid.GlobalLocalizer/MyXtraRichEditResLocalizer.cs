...
{
    public class MyXtraRichEditResLocalizer: XtraRichEditResLocalizer
    {
        public override string GetLocalizedString(XtraRichEditStringId ID)
        {
            return BaseLocalizer.GetLocalizedString<XtraRichEditStringId>((int)ID, base.GetLocalizedString(ID), base.GetType().Name);
            //string enumName = Enum.GetName(typeof(XtraRichEditStringId), ID);
